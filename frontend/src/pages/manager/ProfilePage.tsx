...
interface ManagerProfile {
  ...
  email: string;
  role: string;
  totalVendorsManaged: number;
  totalCustomersManaged: number;
  totalAppointmentsManaged: number;
  createdAt: string;
}
...
const ManagerProfilePage = () => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/manager/profile`, {
        headers: {
                ...
                <CardContent className="space-y-4">
                  <div className="flex justify-between items-center">
                    <div className="flex items-center">
                      ...
                      <span className="text-[#6d4c41]">Vendors Managed</span>
                    </div>
                    <span className="text-[#4e342e] font-bold">{profile.totalVendorsManaged}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <div className="flex items-center">
                      <Users className="w-4 h-4 mr-2 text-[#6d4c41]" />
                      <span className="text-[#6d4c41]">Customers Managed</span>
                    </div>
                    <span className="text-[#4e342e] font-bold">{profile.totalCustomersManaged}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <div className="flex items-center">
                      <Calendar className="w-4 h-4 mr-2 text-[#6d4c41]" />
                      <span className="text-[#6d4c41]">Appointments Managed</span>
                    </div>
                    <span className="text-[#4e342e] font-bold">{profile.totalAppointmentsManaged}</span>
                  </div>
                </CardContent>
              </Card>
            </div>
